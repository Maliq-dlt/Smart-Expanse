...
export default function TopBar() {
  ...
  return (
    ...
    >
      <h1
        className={`font-bold text-[var(--color-on-surface)] font-serif transition-all duration-300 hidden sm:block ${isScrolled ? "text-sm ml-2" : "text-xl"}`}
      >
        SmartExpense
      </h1>
      <h1
        className={`font-bold text-[var(--color-on-surface)] font-serif transition-all duration-300 sm:hidden ${isScrolled ? "text-sm" : "text-lg"}`}
      >
        SE
      </h1>
      ...
      <button
        ...
        className="flex-1 max-w-[200px] mx-2 flex items-center justify-between bg-[var(--color-surface-container)] hover:bg-[var(--color-surface-variant)]/50 text-[var(--color-on-surface-variant)] px-3 py-1.5 rounded-full border border-[var(--color-surface-variant)]/30 transition-colors focus-visible:ring-2 focus-visible:ring-[var(--color-primary)] outline-none"
        aria-label="Cari"
        title="Cari"
      >
        ...
        <span className="text-[10px] font-bold font-mono bg-[var(--color-surface-lowest)] px-1.5 py-0.5 rounded text-[var(--color-outline)] border border-[var(--color-outline)]/20">
          ⌘K
        </span>
      </button>
